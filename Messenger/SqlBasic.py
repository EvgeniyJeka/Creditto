...
class SqlBasic(object):
    ...
    def get_user_name_by_id(self, user_id: int):
        """
        This method can be used to fetch user data from 'users' SQL table by user ID.
        :param user_id: int
        :return: dict
        """
        try:
            ...
            query = db.select(table_).where(table_.columns.id == user_id)
            ResultProxy = self.cursor.execute(query)

            result = ResultProxy.fetchall()
            return self.pack_to_dict(result, "users")
